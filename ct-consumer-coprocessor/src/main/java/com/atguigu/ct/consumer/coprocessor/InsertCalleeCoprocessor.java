 ...
 * 1, 创建类
 * 2, 让表找到协处理类(和表有关联)
 * 3, 将项目打成jar包,发布到hbase中(关联的jar也需要发布), 并且需要分发
 *    本模块的jar : /opt/module/hbase/lib
 *    关联的jar:  /opt/module/hbase/lib     同一个目录下
 *
...
public class InsertCalleeCoprocessor extends BaseRegionObserver {
    ...
    public void postPut(ObserverContext<RegionCoprocessorEnvironment> e, Put put, WALEdit edit, Durability durability) throws IOException {
        // 获取表
        Table table = e.getEnvironment().getTable(TableName.valueOf(Names.TABLE.getValue().toString()));
        // 主叫用户的rowkey
